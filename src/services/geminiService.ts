...
export const analyzeProductLabel = async (
  ...
  return safeParse<ScanResult>(response.text, {} as ScanResult);
};
...
export const getIngredientDetails = async (
  ...
  return safeParse<{ commonUses: string; healthImplications: string; safetyVerdict: string }>(
    response.text,
    { commonUses: '', healthImplications: '', safetyVerdict: '' }
  );
};
...
export const analyzeManualItem = async (
  ...
  return safeParse<{ safetyScore: number; status: SafetyStatus; expiryDate: string; calories?: string; safetyNote: string }>(
    response.text,
    { safetyScore: 0, status: SafetyStatus.CAUTION, expiryDate: 'N/A', safetyNote: '' }
  );
};
